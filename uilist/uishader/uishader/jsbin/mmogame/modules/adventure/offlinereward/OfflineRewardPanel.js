...
(function (offlinereward) {
    var OfflineRewardPanel = (function (_super) {
        ...
        OfflineRewardPanel.prototype.refresh = function ($data) {
            ...
            //预先绘制获得的武器数目
            var equbase = Math.floor(Number($data.reserve) / ((100 + viptab.riskReward) / 100));
            ...
            for (var j = 0; j < rewardAry.length; j++) {
                if (rewardAry[j][0] == 3) {
                    ...
                    var base = Math.floor(rewardAry[j][1] / ((100 + viptab.riskReward) / 100));
                ...
                if (rewardAry[j][0] == -2) {
                    ...
                    var total = rewardAry[j][1] + Number($data.reserve);
                    var base = Math.floor(total / ((100 + viptab.riskReward) / 100));
            ...
            var hour = Math.floor($data.reserve2 / 3600);
            var min = Math.ceil(($data.reserve2 / 60) - (hour * 60));
        ...
        OfflineRewardPanel.prototype.getIndex = function ($ary) {
