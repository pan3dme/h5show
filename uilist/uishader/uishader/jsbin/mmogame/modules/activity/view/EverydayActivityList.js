...
(function (activity) {
    var EverydayActivityList = (function (_super) {
        ...
        EverydayActivityList.prototype.tickRefreshState = function () {
            if (activity.ActivityModel.getInstance().compareAandB(this._aryList, activity.ActivityType.ALL)) {
                ModuleEventManager.dispatchEvent(new activity.ActivityEvent(activity.ActivityEvent.ACTIVITY_TABREDPOINT_EVENT));
            }
        };
        EverydayActivityList.prototype.refreshDataByNewData = function ($type) {
            //通过type，获得所对应的列表
            this._aryList = activity.ActivityModel.getInstance().getList($type);
        ...
        EverydayActivityList.prototype.show = function () {
            if (!this.hasStage) {
                UIManager.getInstance().addUIContainer(this);
                this.refreshDataByNewData(activity.ActivityType.ALL);
                this.addTick();
            }
        };
    ...
    var EverydayActivityRender = (function (_super) {
        ...
        EverydayActivityRender.prototype.drawBtn = function ($data) {
            ...
            if ($data.state == -1) {
                ...
                //正在进行中
                if ($data.canreceive) {
                    var imgUseRect1 = this.parentTarget.baseAtlas.getRec("Btn_1");
                }
                else {
