...
(function (strengthgem) {
    var StrengTab2 = (function (_super) {
        ...
        StrengTab2.prototype.drawEqu = function ($ui, $select, $type) {
            ...
            if (equData) {
                strengthgem.StrengUtil.setEquIcon($ui, geteqiconIconUrl(equData.entryData.icon), stateary, $select, $type, equData.entryData.quality, equData.entryData.level);
            }
            else {
        ...
        StrengTab2.prototype.drawBtn = function ($data, $index) {
            ...
            if ($data.state == 1) {
                ...
                for (var i = 0; i < tab_gem_part.gem_array.length; i++) {
                    if (tab_gem_part.unlock_refine_lv[i] > strengthgem.NewStrengModel.getInstance().getrefiningvo($data.partid).partlevstar[0]) {
                        LabelTextFont.writeSingleLabel(this._baseRender.uiAtlas, this.UnlockUIAry[$index].skinName, strengthgem.StrengUtil.equProp[$data.partid - 1] + "部位精炼达到" + (tab_gem_part.unlock_refine_lv[i] + 1) + "段解锁", 14, TextAlign.RIGHT, ColorType.Brown7a2f21);
                        break;
                    }
                    if (tab_gem_part.unlock_strength_lv[i] > strengthgem.NewStrengModel.getInstance().getstrengvo($data.partid).partlev) {
                        LabelTextFont.writeSingleLabel(this._baseRender.uiAtlas, this.UnlockUIAry[$index].skinName, strengthgem.StrengUtil.equProp[$data.partid - 1] + "部位强化+" + tab_gem_part.unlock_strength_lv[i] + "解锁", 14, TextAlign.RIGHT, ColorType.Brown7a2f21);
                        break;
                    }
                }
            }
