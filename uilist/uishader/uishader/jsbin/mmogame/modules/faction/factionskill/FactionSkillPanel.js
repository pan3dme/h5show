...
(function (faction) {
    var FactionSkillPanel = (function (_super) {
        ...
        FactionSkillPanel.prototype.drawSkill = function ($data) {
            ...
            var idx = 1000 * $data.id + $data.lev + 1;
            var tabObj = TableData.getInstance().getData(TableData.tb_faction_skill_lvup, idx);
            if (tabObj) {
                LabelTextFont.writeSingleLabel(this._baseUiAtlas, this._skillCost.skinName, ColorType.Brown7a2f21 + "消耗个人家族贡献：" + ColorType.color4392ff + tabObj.cost[0][1], 16, TextAlign.LEFT);
            }
            else {
