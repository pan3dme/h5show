function getmarmosetUrl($str: string): string {
...
class ProdkarenResModel {
    ...
    public flectAlphaByUrl(rgburl: string, alphaurl: string, $fun: Function): void {
        LoadManager.getInstance().load(Scene_data.fileRoot +rgburl, LoadManager.XML_TYPE,
            ($rgbstr: string) => {
                var $rgbItem: Array<number> = this.getArrByStr($rgbstr)
                LoadManager.getInstance().load(Scene_data.fileRoot +alphaurl, LoadManager.XML_TYPE,
                    ($alphastr: string) => {
