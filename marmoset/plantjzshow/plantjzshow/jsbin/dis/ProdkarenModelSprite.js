...
var ProdkarenModelSprite = /** @class */ (function (_super) {
    ...
    function ProdkarenModelSprite() {
        ...
        */
        ...
        ProdkarenResModel.getInstance().loadBuffByTxtUrl(Scene_data.fileRoot + getmarmosetUrl("model.txt"), function ($buff) {
    ...
    ProdkarenModelSprite.prototype.loadTexture = function () {
        var _this = this;
        ProdkarenResModel.getInstance().loadTexturelByBoldUrl(getmarmosetUrl("pic703432.txt"), function ($texture) {
            _this._uvTextureRes = $texture;
        });
        //ProdkarenResModel.getInstance().loadTexturelByBoldUrl(getmarmosetUrl("pic598211.txt"), ($texture: TextureRes) => {
        //    this.reflectivityRes = $texture
        //});
        //ProdkarenResModel.getInstance().loadTexturelByBoldUrl(getmarmosetUrl("reflectivityalpha.txt"), ($texture: TextureRes) => {
        //    this.reflectivityalphaRes = $texture
        //});
        ProdkarenResModel.getInstance().loadTexturelByBoldUrl(getmarmosetUrl("pic535317.txt"), function ($texture) {
            _this.normalTextRes = $texture;
        });
        ProdkarenResModel.getInstance().loadSkyTextureByUrl(getmarmosetUrl("picsky.txt"), function ($texture) {
            _this.tSkySpecular = $texture;
        });
        ProdkarenResModel.getInstance().flectAlphaByUrl(getmarmosetUrl("pic598211.txt"), getmarmosetUrl("reflectivityalpha.txt"), function ($texture) {
    ...
    ProdkarenModelSprite.prototype.update = function () {
        if (this.kkkDisplay3DSprite && this.kkkDisplay3DSprite.objData) {
            if (this.tSkySpecular && this._uvTextureRes && this.normalTextRes && this.imageDataReflecRes && this.modelBuff) {
                ...
                */
                ...
                var empteyM = new Matrix3D();
                Matrix3D.mul(empteyM.m, $pos.m, $view.m);
                var $uModelViewProjectionMatrix = empteyM.clone();
                var $disScale = Scene_data.cam3D.distance / 250;
                ...
                var qv3d = this.mathCamPos($mouseM);
                ...
                Scene_data.context3D.setRenderTexture(this.shader, "tExtras", this._uvTextureRes.texture, 3);
