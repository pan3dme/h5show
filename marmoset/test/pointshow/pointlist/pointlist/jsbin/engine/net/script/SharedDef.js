...
var SharedDef = (function () {
    ...
    SharedDef.ZHUCHENG_FUHUO_X = 118; // 主城复活点
    SharedDef.ZHUCHENG_FUHUO_Y = 145; // 主城复活点
    ...
    SharedDef.MAX_MONEY_CHANGE_TYPE = 123;
    // 特殊包裹类型
    ...
    SharedDef.BAG_TYPE_GEM = 3; // 穿戴宝石的包裹
    ...
    SharedDef.MAX_EQUIP_ATTR = 31;
    // 物品其他属性枚举（物品特殊属性接在MAX_EQUIP_ATTR后面）
    SharedDef.ITEM_OTHER_ATTR_FAIL_TIME = 31; // 失效时间
    SharedDef.ITEM_OTHER_ATTR_STRONG_LV = 32; // 强化等级
    SharedDef.ITEM_OTHER_ATTR_STRONG_FAIL_COUNT = 33; // 强化失败次数
    SharedDef.ITEM_OTHER_ATTR_FORCE = 34; // 战斗力
    // 技能族公共CD类型
    ...
    SharedDef.UNIT_FIELD_ANGER = 118; // 愤怒值
    SharedDef.UNIT_FIELD_USE_RESPAWN_MAPID = 119; // 使用复活丹的地图id
    SharedDef.UNIT_FIELD_PROCESS_TIME = 120; //  进度条读完的时间戳
    SharedDef.UNIT_FIELD_PROCESS_SECONDS = 121; //  读条总共需要时间	
    SharedDef.UINT_FIELD_BOSS_DATA = 122; // boss数据byte 0:boss状态 1:boss前一状态 2:boss索引(云游的是这个用途)
    SharedDef.UINT_FIELD_BOSS_DATA1 = 123; // boss数据byte 0:当前坐标x 1:当前坐标y 2:下一坐标x 3:下一坐标y(云游的是这个用途)
    SharedDef.UINT_FIELD_VIEW_MODE = 123; // 是否是观看模式
    SharedDef.UINT_FIELD_VIRTUAL_CAMP = 124; // 虚拟阵营
    SharedDef.UINT_FIELD_XIANFU_INFO = 125; // 跨服仙府的信息4bytes (0:宝箱数量, 1:死亡次数)
    SharedDef.UINT_FIELD_VIP_LEVEL = 126; // VIP等级
    SharedDef.UINT_FIELD_VIP_TIME_OUT = 127; // VIP超时时间
    SharedDef.UINT_FIELD_VAMPIRIC = 128; // 当前战斗的吸血值
    SharedDef.UINT_INT_FIELD_ILLUSION = 130; // 角色幻化
    SharedDef.UINT_FIELD_BOOS_CUR_SPELLID = 131; // 当前正在释放的蓄力技能idUNIT_INT_FIELD_APPEARANCE
    SharedDef.UINT_FIELD_BOOS_CUR_SPELL_TIME = 132; // 前一个技能结束的时间戳
    SharedDef.UINT_FIELD_BOOS_CUR_SPELL_LAST_COUNT = 133; //  持续技能数量
    SharedDef.UINT_INT_FIELD_PASSIVE_SPELL_START = 134; // 被动技能开始
    SharedDef.UINT_INT_FIELD_PASSIVE_SPELL_END = 234; // 被动技能结束
    SharedDef.UINT_INT_FIELD_WINGS_RANK = 234; // 神羽阶数
    SharedDef.UNIT_INT_FIELD_APPEARANCE = 235; //  外观
    SharedDef.MAX_UNIT_FIELD = 236;
    ...
    SharedDef.PLAYER_FIELD_ATTR_RESERVE0 = 77; // 预留属性0
    SharedDef.PLAYER_FIELD_ATTR_RESERVE1 = 79; // 预留属性1
    SharedDef.PLAYER_FIELD_ATTR_RESERVE2 = 81; // 预留属性2
    SharedDef.PLAYER_FIELD_ATTR_RESERVE3 = 83; // 预留属性3
    SharedDef.PLAYER_FIELD_ATTR_RESERVE4 = 85; // 预留属性4
    SharedDef.PLAYER_FIELD_ATTR_RESERVE5 = 87; // 预留属性5
    SharedDef.PLAYER_FIELD_ATTR_RESERVE6 = 89; // 预留属性6
    SharedDef.PLAYER_FIELD_ATTR_RESERVE7 = 91; // 预留属性7
    SharedDef.PLAYER_FIELD_BUFF = 93; // BUFF
    SharedDef.PLAYER_FIELD_BUFF_RESERVE = 143; // BUFF保留参数	
    SharedDef.PLAYER_INT_FIELD_BUFF_GIVER_UINT_GUID = 193; // buff giver的uintGuid
    SharedDef.PLAYER_INT_FIELD_BUFF_EFFECT_ID_START = 243; // buff等级 (byte)
    SharedDef.PLAYER_INT_FIELD_BUFF_EFFECT_ID_END = 293;
    SharedDef.PLAYER_FIELD_SKIN = 293; // 生物变身
    SharedDef.PLAYER_FIELD_BYTES_1 = 294; // 四个字节,分别存放 0怪物分类
    SharedDef.PLAYER_FIELD_FORCE = 295; // 战斗力
    SharedDef.PLAYER_FIELD_EQUIPMENT = 297; // 装备列表，12个32位预留三个
    SharedDef.PLAYER_FIELD_CUR_MOUNT = 317; // 当前坐骑模板id
    SharedDef.PLAYER_FIELD_PEACE_MODE_CD = 318; // 和平模式CD
    SharedDef.PLAYER_FIELD_NOTORIETY = 319; // 2个short(0:战斗模式, 1:恶名值)
    SharedDef.PLAYER_FIELD_BYTES_2 = 320; // 玩家标识,存4个byte[0灰名时间 1付费等级 2免费复活 3玩家称号]
    SharedDef.PLAYER_FIELD_BYTES_3 = 321; // 四个字节,分别存放 0头像,[1发型2可用技能槽数量3下次复活是否有保护buff]
    SharedDef.PLAYER_FIELD_BYTES_4 = 322; // 四个字节,分别存放 (0:频道屏蔽列表,1:预留,2:预留,3:预留)
    SharedDef.PLAYER_FIELD_BYTES_5 = 323; // 0 GM权限, 1, 2 
    SharedDef.PLAYER_FIELD_AUTO_HP = 324; // 大红
    SharedDef.PLAYER_FIELD_AUTO_MP = 326; // 大蓝
    SharedDef.PLAYER_FIELD_VIP_LEVEL = 328; // VIP 等级
    SharedDef.PLAYER_FIELD_VIP_TIME_OUT = 329; // VIP 超时时间
    //  挂机设置
    SharedDef.PLAYER_FIELD_HOOK = 330; // 挂机状态
    SharedDef.PLAYER_FIELD_HOOK_BYTE0 = 331; // 0:槽1技能,1:槽2技能,2:槽3技能,3:槽4技能
    SharedDef.PLAYER_FIELD_HOOK_BYTE1 = 332; // 0:神兵技能,1:怒气技能2:使用银两买药,3:银两不足用元宝买药
    SharedDef.PLAYER_FIELD_HOOK_SHORT = 333; // 0:自动吃药百分比,1:自动使用道具id
    SharedDef.PLAYER_FIELD_HOOK_BYTE3 = 334; // 0:回程复活/原地复活,1:自动使用绑银购买复活丹,2:绑银不足用元宝,3:自动出售某种平直及以下装备
    SharedDef.PLAYER_FIELD_FLAGS = 335; // 主玩家下标flags
    SharedDef.PLAYER_FIELD_FLAGS_END = 343; // 预留8个32位flags
    SharedDef.PLAYER_FIELD_ONLINE_TIME = 343; // 总在线时长(分钟为单位)
    SharedDef.PLAYER_FIELD_ANGER = 344; // 怒气改变
    SharedDef.PLAYER_FIELD_TRIAL_LAYERS = 345;
    SharedDef.PLAYER_INT_FIELD_TRIAL_FINISHED_SECTIONID = 346; // (已经通关的关卡id)
    SharedDef.PLAYER_INT_FIELD_TRIAL_PROCESS = 347; // (0:已经杀的怪,1:总共需要杀的怪)
    SharedDef.PLAYER_FIELD_DECLINE_CHANNEL_BYTE0 = 348; // 拒绝频道A(0系统，1帮派，2组队，3世界)
    SharedDef.PLAYER_FIELD_DECLINE_CHANNEL_BYTE1 = 349; // 拒绝频道B(0:附近)
    SharedDef.PLAYER_SCENED_INT_FIELD_PLAYER_JUMP_CD = 350; // 跳跃CD
    SharedDef.PLAYER_INT_FIELD_SYSTEM_MAIL_ID = 351; // 当前领取的系统邮件的id
    SharedDef.PLAYER_FIELD_DIVINE_NUM = 352; // 神兵数量
    SharedDef.PLAYER_FIELD_DIVINE_FORCE = 353; // 神兵总战斗力
    SharedDef.PLAYER_FIELD_MOUNT_FORCE = 354; // 坐骑总战力
    SharedDef.PLAYER_FIELD_SKILL_FORCE = 355; // 技能战力
    SharedDef.PLAYER_FILED_GEM_FORCE = 356; // 宝石战力
    SharedDef.PLAYER_FIELD_STRENGTH_FORCE = 357; // 强化战力
    SharedDef.PLAYER_FIELD_RANK_LIKE = 358;
    SharedDef.PLAYER_FIELD_USE_RANK_LIKE = 359;
    SharedDef.PLAYER_FIELD_SHOP_LIMIT_START = 360;
    SharedDef.PLAYER_FIELD_SHOP_LIMIT_END = 380;
    SharedDef.PLAYER_EXPAND_INT_FCM_LOGIN_TIME = 380; // 防沉迷，单位：分钟	-1是非防沉迷玩家
    SharedDef.PLAYER_EXPAND_INT_FCM_LOGOUT_TIME = 381; // 防沉迷离线时间
    SharedDef.PLAYER_EXPAND_INT_CREATE_TIME = 382; // 角色创建时间
    SharedDef.PLAYER_EXPAND_INT_LAST_LOGIN_TIME = 383; // 最后登录时间
    SharedDef.PLAYER_EXPAND_INT_LAST_LOGOUT_TIME = 384; // 最后离线时间
    SharedDef.PLAYER_EXPAND_INT_NUMBER_OF_LOGINS = 385; // 累积登录次数
    SharedDef.PLAYER_EXPAND_INT_MONEY = 386; // 各种货币
    SharedDef.PLAYER_EXPAND_INT_MONEY_END = 430;
    SharedDef.PLAYER_EXPAND_INT_USE_FASHION = 430; // 是否启用时装,目前用到20个pos
    SharedDef.PLAYER_EXPAND_INT_XP = 431; // 当前经验，64位
    SharedDef.PLAYER_EXPAND_INT_NEXT_LEVEL_XP = 433; // 下一级经验，64位
    SharedDef.PLAYER_EXPAND_INT_XP_ADDED = 435; //  应用服增加的经验
    SharedDef.PLAYER_EXPAND_INT_XP_PICKED = 437; //  场景服已经领取的应用服增加的经验
    SharedDef.PLAYER_EXPAND_INT_MAP_STATUS = 439; //  地图开启进度
    SharedDef.PLAYER_EXPAND_INT_GAG_END_TIME = 440; // 禁言结束时间
    SharedDef.PLAYER_EXPAND_INT_TO_MAP = 441; // 玩家要传送到的地图
    SharedDef.PLAYER_EXPAND_INT_TO_INSTANCE_ID = 442; // 玩家要传送到的地图实例ID
    SharedDef.PLAYER_EXPAND_INT_TO_POS_X = 443; // 玩家药传送到的坐标
    SharedDef.PLAYER_EXPAND_INT_TO_POS_Y = 444;
    SharedDef.PLAYER_EXPAND_INT_TO_LINE_NO = 445; // 传送线号
    SharedDef.PLAYER_EXPAND_INT_DB_MAP = 446; // 玩家最后一次进入副本前的地图	
    SharedDef.PLAYER_EXPAND_INT_DB_POS_X = 447; // 玩家最后一次进入副本前的坐标
    SharedDef.PLAYER_EXPAND_INT_DB_POS_Y = 448;
    SharedDef.PLAYER_EXPAND_INT_CLIENT_DATA = 449; // 远程存储
    // ////////////////////////////////////////////////////////////////////////
    // 应用服部分	
    SharedDef.PLAYER_APPD_INT_FIELD_FLAG = 549; // 玩家标志位
    SharedDef.PLAYER_APPD_INT_FIELD_FLAG_END = 556; // 结束下标
    SharedDef.PLAYER_APPD_INT_FIELD_DAILY_TIME = 556; //  每天在线时长（min）
    SharedDef.PLAYER_APPD_INT_FIELD_RESET_WEEK = 557; //  每周重置时间
    SharedDef.PLAYER_APPD_INT_FIELD_RESET_DALIY = 558; // 重置的时间点每小时一次
    SharedDef.PLAYER_APPD_INT_FIELD_RECHARGE_SUM = 582; // 累计充值总数
    SharedDef.PLAYER_APPD_INT_FIELD_LAST_RECHARGE_TIME = 583; // 最后一笔充值时间
    SharedDef.PLAYER_APPD_INT_FLELD_JHM_FLAGS = 584; // 激活码flags
    SharedDef.PLAYER_APPD_INT_FLELD_JHM_FLAGS_END = 592; // 激活码flags_end
    // 主背包扩展时间戳
    SharedDef.PLAYER_APPD_INT_FIELD_EXTENSION_END_TIME = 593; // 主包裹扩展结束时间戳
    SharedDef.PLAYER_APPD_INT_FIELD_EXTENSION_REMAIN_TIME = 594; // 主包裹扩展剩余多少时间
    SharedDef.PLAYER_INT_FIELD_BLOCK_COUNT = 596; // 屏蔽列表个数
    SharedDef.PLAYER_INT_FIELD_WORLD_BOSS_JOIN_ID = 597; // 最后一次参加的世界BOSS id
    SharedDef.PLAYER_INT_FIELD_WORLD_BOSS_JOIN_STATE = 598; // 0:最后一次参加的世界BOSS状态, 1:进入的线数,2:死亡次数,3:预留
    // 场景服扩展下标开始了
    SharedDef.PLAYER_SCENED_INT_FLAGS = 599; // 玩家场景服标志位
    //  只显示装备的主动技能
    SharedDef.PLAYER_INT_FIELD_SPELL_START = 607;
    SharedDef.PLAYER_INT_FIELD_SPELL_END = 619;
    // 重要技能的CD
    SharedDef.PLAYER_INT_FIELD_IMPORTANT_SPELL_CD_START = 619;
    SharedDef.PLAYER_INT_FIELD_IMPORTANT_SPELL_CD_END = 659;
    SharedDef.PLAYER_EXPAND_KILL_MONSTER = 659; // 杀怪数
    // 跨服
    SharedDef.PLAYER_APPD_INT_FIELD_KUAFU_WARID = 660; // 跨服场次id
    SharedDef.PLAYER_INT_FIELD_KUAFU_NUMBER = 661; // 0:玩家报名跨服时收到匹配信息中给的编号 1:跨服类型（值参照枚举：EKUAFU_TYPE）
    SharedDef.PLAYER_INT_FIELD_MAIN_QUEST_ID = 662; // 当前主线任务
    SharedDef.PLAYER_INT_FIELD_QUEST_CHAPTER = 663; // 任务章节奖励领取情况
    SharedDef.PLAYER_INT_FIELD_MOUNT_LEVEL = 664; // 4个bytes(0:当前坐骑阶数 1:当前坐骑星级,2:当前坐骑是否骑乘，3:当前幻化id)
    SharedDef.PLAYER_INT_FIELD_DIVINE_ID = 665; // 当前装备的神兵id
    SharedDef.PLAYER_INT_FIELD_PASSIVE_SPELL_START = 666; // 被动技能开始
    SharedDef.PLAYER_INT_FIELD_PASSIVE_SPELL_END = 766; // 被动技能个数
    SharedDef.PLAYER_INT_FIELD_VIRTUAL_CAMP = 766; // 虚拟阵营
    SharedDef.PLAYER_INT_FIELD_WORLD_3V3_SCORE = 767; // 跨服3v3积分
    SharedDef.PLAYER_INT_FIELD_WORLD_3V3_TOTAL_SCORE = 768; // 跨服3v3积分
    SharedDef.PLAYER_INT_FIELD_WORLD_3V3_TREND_INFO = 769; // 胜负走势
    SharedDef.PLAYER_INT_FIELD_DOUJIANTAI_RANK = 770; // 斗剑台排名
    SharedDef.PLAYER_INT_FIELD_DOUJIANTAI_REFRESH_TIME = 771; // 最后一次刷新对手时间戳
    SharedDef.PLAYER_INT_FIELD_ARREARS = 772; // 欠费
    SharedDef.PLAYER_INT_FIELD_DAILY_QUEST_FINISHED = 774; //  每日任务完成数
    SharedDef.PLAYER_INT_FIELD_BAG_SORT_TIME = 775; // 整理背包时间戳
    // 已开启功能的记录
    SharedDef.PLAYER_INT_FIELD_OPEN_MENU_INDEX = 776;
    SharedDef.PLAYER_INT_FIELD_OPEN_MENU_START = 777;
    SharedDef.PLAYER_INT_FIELD_OPEN_MENU_END = 877;
    // 已购家族商店物品的记录
    SharedDef.PLAYER_INT_FIELD_BUYED_FACTION_SHOP = 878; // 已购家族商店物品开始
    SharedDef.PLAYER_INT_FIELD_BUYED_FACTION_SHOP_END = 886; // 已购家族商店物品结束
    // 记录强制引导ID 
    SharedDef.PLAYER_INT_FIELD_GUIDE_ID_LAST = 887; // 上次完成的引导
    SharedDef.PLAYER_INT_FIELD_GUIDE_ID_NOW = 888; // 当前进行中的引导
    // 修炼场相关
    SharedDef.PLAYER_INT_FIELD_CULTIVATION_LAST_PLUNDER_TIME = 889; // 上次挑战时间戳
    SharedDef.PLAYER_INT_FIELD_CULTIVATION_LAST_REFRESH_TIME = 890; // 上次刷新对手时间戳
    SharedDef.PLAYER_INT_FIELD_CULTIVATION_TODAY_PLUNDER_EXP = 891; // 当日累计掠夺经验
    SharedDef.PLAYER_INT_FIELD_CULTIVATION_TODAY_PLUNDER_CHEST = 892; // 当日累计掠夺宝箱数
    SharedDef.PLAYER_INT_FIELD_CULTIVATION_TODAY_PURCHASE_COUNT = 893; // 当日购买次数
    SharedDef.PLAYER_INT_FIELD_CULTIVATION_LEFT_PLUNDER_COUNT = 894; // 当前剩余挑战次数
    SharedDef.PLAYER_INT_FIELD_CULTIVATION_PLUNDER_RECORD_INDEX = 895; // 修练场修炼记录下标
    SharedDef.PLAYER_INT_FIELD_CULTIVATION_TOTAL_BEEN_PLUNDER_COUNT = 896; // 本轮被掠夺成功次数
    SharedDef.PLAYER_INT_FIELD_CULTIVATION_TODAY_TOTAL_PLUNDER_COUNT = 897; // 本日总掠夺次数
    SharedDef.PLAYER_INT_FIELD_CULTIVATION_START_TIME = 898; // 修炼开始时间
    SharedDef.PLAYER_INT_FIELD_CULTIVATION_LOST_INFO = 899; // 修炼宝箱丢失信息
    // 登陆大礼相关
    SharedDef.PLAYER_INT_FIELD_LOGIN_ACTIVITY_TOTAL_DAYS = 900; // 登陆总天数
    SharedDef.PLAYER_INT_FIELD_LOGIN_ACTIVITY_REWARD_STATUS_START = 901; // 领取状态开始
    SharedDef.PLAYER_INT_FIELD_LOGIN_ACTIVITY_REWARD_STATUS_END = 903; // 领取状态结束
    //  非强制引导ID
    SharedDef.PLAYER_INT_FIELD_OPTIONAL_GUIDE_START = 904; // 非强制引导开始
    SharedDef.PLAYER_INT_FIELD_OPTIONAL_GUIDE_END = 964; //  非强制引导结束
    SharedDef.PLAYER_INT_FILED_LEAVE_RISK_TIME = 964; // 离开冒险世界的时间戳
    SharedDef.PLAYER_INT_FILED_LEAVE_RISK_SUIT_SCORE = 965; // 是否已经领取不在冒险世界的挂机奖励
    SharedDef.PLAYER_INT_FILED_FACTION_BUILDING_SPEEDUP_DAILY_COUNT = 966; // 家族建筑升级加速每日次数
    SharedDef.PLAYER_INT_FILED_FACTION_DONATE_GIFT_EXCHANGE_DAILY_COUNT = 967; // 家族宝库贡献礼物兑换每日次数
    SharedDef.PLAYER_INT_FILED_CHARM_POINT = 968; // 魅力值
    SharedDef.PLAYER_INT_FILED_FACTION_GIFT_NEXT_COUNT_ID = 970; // 玩家礼物历史记录下一个count_id
    SharedDef.PLAYER_INT_FILED_FACTION_GIFT_START = 971; // 玩家礼物历史记录开始
    SharedDef.PLAYER_INT_FILED_FACTION_GIFT_END = 1721;
    SharedDef.PLAYER_INT_FIELD_CREATE_ICON = 1721; // 待创建的帮派icon
    SharedDef.PLAYER_INT_FIELD_FACTION_GIFT_POINT_COUNT = 1722; // 帮派魅力赠送总记数
    SharedDef.PLAYER_INT_FIELD_FACTION_GIFT_SEND_COUNT = 1724; // 帮派礼物赠送总记数
    SharedDef.PLAYER_INT_FIELD_FACTION_GIFT_BEEN_THANK_COUNT = 1725; // 帮派礼物赠送被女王感谢记数
    SharedDef.PLAYER_INT_FIELD_MASS_BOSS_BUYED_TIMES = 1726; // 已经购买次数
    SharedDef.PLAYER_INT_FIELD_MASS_BOSS_TIMES = 1727; // 全民boss次数
    SharedDef.PLAYER_INT_FIELD_MASS_BOSS_CD = 1728; // 全民boss回复cd(0:表示已满了, 只有次数不满的情况才有cd)
    SharedDef.PLAYER_INT_FIELD_TALISMAN_FORCE = 1729; // 法宝总战力
    SharedDef.PLAYER_INT_FIELD_WINGS_FORCE = 1730; // 神羽总战力
    SharedDef.PLAYER_INT_FIELD_GROUP_INSTANCE_CLEAR_FLAG = 1731; // 组队副本首次通关flag
    SharedDef.PLAYER_INT_FIELD_MERIDIAN_FORCE = 1732; // 经脉修炼战力
    SharedDef.PLAYER_INT_FIELD_WINGS_RANK = 1733; // 神羽阶数
    SharedDef.PLAYER_INT_FIELD_FACTION_BOSSDEFENSE_TICKETS = 1734; // 家族首领挑战剩余次数
    SharedDef.PLAYER_INT_FIELD_FACTION_TOWER_CLEAR_FLOOR = 1735; // 家族无尽远征 历史挑战层数
    SharedDef.PLAYER_INT_FIELD_FACTION_TOWER_FLAG = 1736; // 0 扫荡状态 1~15 宝箱领取状态
    SharedDef.PLAYER_INT_FIELD_QUALIFY_SCORE = 1737; //  排位赛积分
    SharedDef.PLAYER_INT_FIELD_FACTION_SKILL_LV_START = 1738; // 家族技能等级开始 0:已学习等级 uint16 1:当前生效等级
    SharedDef.PLAYER_INT_FIELD_FACTION_SKILL_LV_END = 1758; // 家族技能等级结束
    SharedDef.PLAYER_INT_FIELD_APPEARANCE = 1758; //  外观(0:武器,1:衣服)
    SharedDef.PLAYER_INT_FIELD_WEAPON_FORCE = 1759; //  武器外观战力
    SharedDef.PLAYER_INT_FIELD_CLOTH_FORCE = 1760; //  衣服外观战力
    SharedDef.PLAYER_INT_FIELD_POKEDEX_FORCE = 1761; //  图鉴战力
    SharedDef.PLAYER_INT_FIELD_TITLE_FORCE = 1762; //  称号战力
    SharedDef.PLAYER_INT_FIELD_RENAME_TIMES = 1763; //  改名次数
    SharedDef.PLAYER_INT_FIELD_REVENGE_TIMES = 1764; // 当日剩余复仇次数
    SharedDef.PLAYER_INT_FIELD_REVENGE_BUY_TIMES = 1765; // 当日复仇购买次数
    SharedDef.PLAYER_INT_FIELD_WINGS_STAR = 1766; // 神羽星数
    SharedDef.PLAYER_INT_FIELD_VIPGIFT_FLAG = 1767; // vip礼包购买状态
    SharedDef.PLAYER_APPD_INT_FIELD_CONSUME_SUM = 1768; // 累计消费元宝
    // ////////////////////////////////////////////////////////////////////////////////////////////////////////
    // 字符串部分
    ...
    SharedDef.MAX_MASS_BOSS_COUNT = 10;
    ...
    SharedDef.GLOBALVALUE_INT_FIELD_MASS_BOSS_END = 312;
    SharedDef.GLOBALVALUE_INT_FIELD_LOTTERY_RECORD_CURSOR_START = 312; //  抽奖记录游标
    SharedDef.GLOBALVALUE_INT_FIELD_LOTTERY_RECORD_CURSOR_END = 322;
    SharedDef.GLOBALVALUE_INT_FIELD_ACTIVITIES_RUNNING_START = 322; //  活动运行id 开始
    SharedDef.GLOBALVALUE_INT_FIELD_ACTIVITIES_RUNNING_END = 352; //  活动运行id 结束
    ...
    SharedDef.TRIAL_INSTANCE_FIELD_BOSS_REFRESHED = 46; // 是否已经刷怪
    // 家族场景
    ...
    SharedDef.MODULE_BOSS_WORLD_BOSS = 2; // 世界BOSS
    ...
    SharedDef.MODULE_FIRST_RECHARGE_ALL = 1; // 首冲hu
