...
(function (marmoset) {
    ...
    Material.prototype.bind = function (a) {
			...
			m.uniform4fv(n.uLightPositions, c.positionBuffer)
			m.uniform3fv(n.uLightDirections, c.directionBuffer)
			m.uniform3fv(n.uLightColors, c.colors)
			m.uniform3fv(n.uLightParams, c.parameters)
			m.uniform3fv(n.uLightSpot, c.spot)
			m.uniform2f(n.uShadowKernelRotation, 0.5 , 0.5 )
    ...
    function Mesh(a, b, c) {
	   ...
	   var outIndexStr=""
		for(var i =0;i<d.length/2;i++){
            ...
            a.bufferData(a.ARRAY_BUFFER, new Float32Array(vertexArr), a.STATIC_DRAW);
    ...
    TextureCache.prototype.fromFilesMergeAlpha = function (a, b, c) {
		if(b&&b.data.length==1645617){
            ...
            if (void 0 !== e) return e;
            var f = this.gl;
            this.mergeShader || (this.mergeShader = new Shader(this.gl), this.mergeShader.build(
			"precision highp float; varying vec2 c; attribute vec2 pos; void main(){ gl_Position.xy = 2.0*pos-vec2(1.0); gl_Position.zw = vec2(0.5,1.0); c=pos; }", "precision highp float; varying vec2 c; uniform sampler2D tRGB,tA; void main(){ gl_FragColor.xyz=texture2D(tRGB,c).xyz; gl_FragColor.w=texture2D(tA,c).x; }"
