var Physics = (function () {
    ...
    Physics.creatWorld = function () {
        ...
        Physics.world.gravity = this.Vec3dW2C(new Vector3D(0, -1000, 0));
    ...
    Physics.QuaternionCollisionToH5 = function ($q) {
        return new Quaternion($q.x, $q.z, $q.y, $q.w);
    };
    Physics.QuaternionC2Vec3dW = function ($q) {
        var $v = new CANNON.Vec3(0, 0, 0);
        $q.toEuler($v);
    ...
    //通过objs的数据来生存一个body
    Physics.makeBuildBodyMesh = function ($dis, $collisionItemVo) {
        var $bodyMesh = Physics.getBodyMesh(new Vector3D($dis.x, $dis.y, $dis.z), 100);
        var m = new Matrix3D;
