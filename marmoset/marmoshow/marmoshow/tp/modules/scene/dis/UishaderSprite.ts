...
class UishaderSprite extends BaseDiplay3dSprite {
    ...
    private _nrmTextureRes: TextureRes;
    private _reflectivity: TextureRes;
    private _tSkySpecular: TextureRes
    ...
    private loadAlphaJpg($rgbUrl: string, $alphaUrl: string): void {
        LoadManager.getInstance().load(Scene_data.fileRoot + $rgbUrl, LoadManager.IMG_TYPE, ($rgbImg: any) => {
            LoadManager.getInstance().load(Scene_data.fileRoot + $rgbUrl, LoadManager.IMG_TYPE, ($alphaImg: any) => {
                var ctx: CanvasRenderingContext2D = UIManager.getInstance().getContext2D($rgbImg.width, $rgbImg.height, false);
                ctx.drawImage($rgbImg, 0, 0, $rgbImg.width, $rgbImg.height);
                var rgbimgData: ImageData = ctx.getImageData(0, 0, $rgbImg.width, $rgbImg.height);

                ctx.drawImage($alphaImg, 0, 0, $rgbImg.width, $rgbImg.height);
                var alphaimgData: ImageData = ctx.getImageData(0, 0, $alphaImg.width, $alphaImg.height);
