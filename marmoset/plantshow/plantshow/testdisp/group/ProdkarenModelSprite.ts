...
class ProdkarenModelSprite extends BaseDiplay3dSprite {
    ...
    constructor() {
        ...
        ProdkarenResModel.getInstance().loadBuffByTxtUrl(Scene_data.fileRoot +getmarmosetUrl("model.txt"), ($buff: WebGLBuffer) => {
            this.modelBuff = $buff;
        });
    ...
    protected loadTexture(): void {
        ...
        ProdkarenResModel.getInstance().loadTexturelByBoldUrl(getmarmosetUrl("pic703432.txt"), ($texture: TextureRes) => {
        ...
        //});
        ProdkarenResModel.getInstance().loadTexturelByBoldUrl(getmarmosetUrl("pic535317.txt"), ($texture: TextureRes) => {
            this.normalTextRes = $texture
        });
        ProdkarenResModel.getInstance().loadSkyTextureByUrl(getmarmosetUrl("picsky.txt"), ($texture: TextureRes) => {
            this.tSkySpecular = $texture
        });

        ProdkarenResModel.getInstance().flectAlphaByUrl(getmarmosetUrl("pic598211.txt"), getmarmosetUrl("reflectivityalpha.txt"), ($texture: TextureRes) => {
